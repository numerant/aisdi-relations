...
unsigned int Usember::getMaxEmailsInMonth ()
{
    ...
    for (int i = 0; i < 12; i++)
    {
        temp = getEmailsReceivedInMonth(i+1);
        if (temp > maxSum)
            maxSum = temp;
        temp = getEmailsSentInMonth(i+1);
        if (temp > maxSum)
            maxSum = temp;
...
unsigned int  Usember::getForwardsCount(Email* email, bool mode)
{
	unsigned int emailsSum = 0;
	Email* tempE;
	string subject;
	for (unsigned int i = 0; i < this->sendMailCount(); i++)
	{
		...
		subject = tempE->getSubject();
		if (mode)
			subject = subject.substr(5, subject.size()-5);
		if ( (subject == email->getSubject()) && (tempE->getIsForwarded()) )
			emailsSum++;
	}
	return emailsSum;
}
...
Email*  Usember::getForward(unsigned int position, Email* email, bool mode)
{
	unsigned int emailsSum = 0;
	Email* tempE;
	string subject;
